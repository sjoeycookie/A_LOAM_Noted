...
//这个源文件的作用是将kitti数据转换成ros下的话题数据，并可选择保存为bag文件
int main(int argc, char** argv)
{
    ros::init(argc, argv, "kitti_helper"); //该节点名称，且初始化
    ros::NodeHandle n("~");     //使用节点句柄nh("~")和nh的区别：发布的话题的命名空间不同。 nh("~")发布的话题名字为：节点名字+话题名字；nh发布的话题名字为:话题名字
    ...
    while (std::getline(timestamp_file, line) && ros::ok())
    {
        ...
        //这里调用读雷达数据的函数
        std::vector<float> lidar_data = read_lidar_data(lidar_data_path.str());  //read_lidar_data()是函数
